...
def test_search_migratable_resources_rest_bad_request(request_type=migration_service.SearchMigratableResourcesRequest):
    client = MigrationServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
def test_search_migratable_resources_rest_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        ...
        return_value = migration_service.SearchMigratableResourcesResponse.to_json(migration_service.SearchMigratableResourcesResponse())
...
def test_batch_migrate_resources_rest_bad_request(request_type=migration_service.BatchMigrateResourcesRequest):
    client = MigrationServiceClient(
        credentials=ga_credentials.AnonymousCredentials(),
        transport="rest"
    )
...
@pytest.mark.asyncio
async def test_search_migratable_resources_rest_asyncio_bad_request(request_type=migration_service.SearchMigratableResourcesRequest):
    if not HAS_ASYNC_REST_EXTRA:
        pytest.skip("the library must be installed with the `async_rest` extra to test this feature.")
    client = MigrationServiceAsyncClient(
        credentials=async_anonymous_credentials(),
...
async def test_search_migratable_resources_rest_asyncio_interceptors(null_interceptor):
    ...
    transport = transports.AsyncMigrationServiceRestTransport(
        credentials=async_anonymous_credentials(),
        interceptor=None if null_interceptor else transports.AsyncMigrationServiceRestInterceptor(),
        )
    client = MigrationServiceAsyncClient(transport=transport)

    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(transports.AsyncMigrationServiceRestInterceptor, "post_search_migratable_resources") as post, \
        mock.patch.object(transports.AsyncMigrationServiceRestInterceptor, "pre_search_migratable_resources") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migration_service.SearchMigratableResourcesRequest.pb(migration_service.SearchMigratableResourcesRequest())
        transcode.return_value = {
        ...
        await client.search_migratable_resources(request, metadata=[("key", "val"), ("cephalopod", "squid"),])

        pre.assert_called_once()
        post.assert_called_once()

@pytest.mark.asyncio
async def test_batch_migrate_resources_rest_asyncio_bad_request(request_type=migration_service.BatchMigrateResourcesRequest):
    if not HAS_ASYNC_REST_EXTRA:
        pytest.skip("the library must be installed with the `async_rest` extra to test this feature.")
    client = MigrationServiceAsyncClient(
        credentials=async_anonymous_credentials(),
...
async def test_batch_migrate_resources_rest_asyncio_interceptors(null_interceptor):
    ...
    with mock.patch.object(type(client.transport._session), "request") as req, \
        mock.patch.object(path_template, "transcode")  as transcode, \
        mock.patch.object(operation.Operation, "_set_result_from_operation"), \
        mock.patch.object(transports.AsyncMigrationServiceRestInterceptor, "post_batch_migrate_resources") as post, \
        mock.patch.object(transports.AsyncMigrationServiceRestInterceptor, "pre_batch_migrate_resources") as pre:
        pre.assert_not_called()
        post.assert_not_called()
        pb_message = migration_service.BatchMigrateResourcesRequest.pb(migration_service.BatchMigrateResourcesRequest())
        transcode.return_value = {
        ...
        await client.batch_migrate_resources(request, metadata=[("key", "val"), ("cephalopod", "squid"),])
